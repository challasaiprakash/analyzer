...
COMPARISON_COLORS = ["#00E6FF", "#F59E0B", "#22C55E", "#A855F7", "#EF4444", "#F97316", "#14B8A6", "#EAB308"]
...
def render_selected_graph(graph_type: str, signal: np.ndarray, sample_rate: int):
    if graph_type == "Amplitude envelope":
        st.plotly_chart(envelope_figure(signal, sample_rate), use_container_width=True)
    elif graph_type == "Waveform":
        st.plotly_chart(waveform_figure(signal, sample_rate, "Waveform", color=SECONDARY_COLOR), use_container_width=True)
    elif graph_type == "Frequency domain":
        st.plotly_chart(frequency_figure(signal, sample_rate), use_container_width=True)
    elif graph_type == "Cepstrum":
        st.plotly_chart(cepstrum_figure(signal, sample_rate), use_container_width=True)
    elif graph_type == "Autocorrelation":
        st.plotly_chart(autocorr_figure(signal, sample_rate), use_container_width=True)
    elif graph_type == "Spectrogram":
        st.image(spectrogram_png(signal, sample_rate), use_container_width=True)
    elif graph_type == "Compressed spectrogram":
        st.image(pseudo_mel_png(signal, sample_rate), use_container_width=True)
    elif graph_type == "Scalogram":
        st.image(scalogram_png(signal, sample_rate), use_container_width=True)


def render_multi_graph_output(graph_type: str, graph_items):
    if graph_type in {"Amplitude envelope", "Waveform", "Frequency domain"}:
        st.plotly_chart(comparison_figure(graph_items, graph_type), use_container_width=True)
        return
    ...
    if len(graph_items) > 1:
        ...
        for index, item in enumerate(graph_items):
            with image_columns[index % len(image_columns)]:
                ...
                render_selected_graph(graph_type, item["signal"], item["sample_rate"])
    else:
        item = graph_items[0]
        render_selected_graph(graph_type, item["signal"], item["sample_rate"])
...
def render_single_audio_tab():
    ...
    with waveform_left:
        st.plotly_chart(waveform_figure(signal, sample_rate, "Full Waveform"), use_container_width=True)
    with waveform_right:
        st.plotly_chart(waveform_figure(segment, sample_rate, "Selected Segment", color=SECONDARY_COLOR), use_container_width=True)

    render_selected_graph(graph_type, segment, sample_rate)
...
def render_multi_audio_tab():
    ...
    for panel_index, panel in enumerate(panels):
        ...
        render_multi_graph_output(graph_type, graph_items)
